...
export const updatePostsFromResponse = (response: PostListResponseDto): Array<PostState> => {
	const totalResponsePosts: Array<EvaluationPostDto> = [];
	response.user[0].followers.forEach(
		person => totalResponsePosts.push(...person.following_user.evaluation_posts));
	totalResponsePosts.push(...response.user[0].evaluation_posts);

	const posts = totalResponsePosts.map(dto => <PostState>{
		id: dto.id,
		postOwner: {
		...
		title: dto.title,
		body: dto.body,
		images: dto.post_images.map(imageDto => ({
			id: imageDto.id,
			url: imageDto.url,	
		}) as ImageState),
		hotel: {
