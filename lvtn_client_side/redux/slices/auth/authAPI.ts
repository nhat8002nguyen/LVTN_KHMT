...
export const addGoogleAccountToDBIfNotExist = async (user: UserRequestDto): Promise<SyncUserResponseDto> => {
	try {
		const checkAccountRes = await harusaAxios.get(
			"/users/google-account-id",
			{ 
				params: {
		...
		const syncAccountRes = await harusaAxios.post("/auth/register", null, { params: {
